...
- [skill-invocation-policy.md](skill-invocation-policy.md) — the implemented Skill invocation policy (ADR-030): canonical `invoke:` model, per-harness support matrix (current official behavior), routing/exact-coverage rules. Replaces the former Command capability spec.
- [agents.md](agents.md)
...
| Skill invocation policy | **IMPLEMENTED** | One Skill capability, `invoke: {model, user}` as its portable semantics (ADR-030). Translated natively into Claude Code (`disable-model-invocation`, `user-invocable`), Codex (`agents/openai.yaml` policy sidecar; `user: false` honestly Degraded) and OpenCode V2 (`metadata.opencode/autoinvoke`, `slash`); ADAPTED to Antigravity (no explicit-only mechanism; the loss is named in the plan evidence). A vendor Command is a projection detail only. See [skill-invocation-policy.md](skill-invocation-policy.md). Exposed under stable namespaced invocation labels `<plugin>:<skill>` (ADR-026). |
...
| Commands / Actions | **Merged into Skills** (official) | Distinct from Skills; ~29 built-ins; third-party package extensibility unconfirmed | Distinct from Skills; markdown + shell-injection (`` !`cmd` ``) | Distinct from Skills; TOML, package-native, lowest-precedence namespacing | Not converging on one contract; Claude's own docs say the concept collapsed into Skills. Vendor taxonomy only — UZE models the portable invariant (`invoke:` policy), never the vendor primitive. See [skill-invocation-policy.md](skill-invocation-policy.md). |
...
| Skill invocation policy | **FITS, implemented** | No new `CapabilityKind`: the Skill stays `AgentSkill` and its `invoke:` policy is parsed once at discovery into a small vendor-neutral `SkillInvocationPolicy` carried on `Resource`. Integrations translate the policy into vendor encodings at delivery (generated wrappers under `$UZE_HOME`, never Store rewrites); `ManagedFile` remains a generic primitive. The intermediate `Command` kind (ADR-025) was removed by ADR-030 before any released state depended on it. |
