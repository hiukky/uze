...
| Commands | **N/A** | Research does not support modeling Commands as a UZE capability at all (see [commands.md](commands.md)); this is not a Core-fit question. |
...
| `Action` | **REMOVE_LATER candidate** | No harness researched here (or in the prior ecosystem pass) exposes a capability literally matching "Action" distinct from Commands/Skills/Tools. Low urgency — it costs nothing unimplemented, but it should not survive a future cleanup pass without new evidence. |
