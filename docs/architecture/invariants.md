...
### An icon has the cell after it to itself, and a set of them is one size

Every Nerd Font build reserves an icon one cell, and the plain and Propo
builds paint it across about 1.7. So wherever UZE draws a glyph from a
patched font, the cell after it is blank and in the icon's own ground. The
overflow lands on space UZE reserved, never on the text, border or chip
edge beside it. Whether a glyph is an icon is read from the glyph itself
(a private-use codepoint), so a set drawn in plain Unicode reserves
nothing. The rule is checked on drawn frames, not on call sites.

The bundled `nerd` set is regenerated by measurement against a pinned
catalog of Nerd Fonts. Every icon it declares stays within ±13% of the
set's median size in every build of every font there, so switching fonts
never leaves one icon large and its neighbour small. What real terminals
make of it is measured from their pixels by the Rendering Lab
(`conformance/rendering/`), and the public catalog on the appearance page
is generated from that evidence.

> `src/ui/tests.rs::every_icon_on_a_management_screen_has_its_slot`
> `src/ui/orchestrator/tests.rs::every_icon_in_the_workspace_has_its_slot`
> `src/ui/theme.rs::an_icon_is_checked_for_what_follows_it_and_in_which_ground`
> `conformance/rendering/metrics.py` (the `Rendering` workflow)
> `conformance/rendering/generate_nerd.py --check` (the `Rendering` workflow)
> `conformance/rendering/catalog.py docs --check` (`ci.yml`, Python)
