...
| 3 | Five production `unsafe` blocks without `// SAFETY:` (`prompt_history.rs` localtime, `runtime.rs` getuid ×2 and kill, `main.rs` SIGPIPE) | Project rule and the brief's; `getuid` twice can be one helper. | S |
...
| 7 | `model.rs` `Route::index` bare `unwrap` | An invariant (every route is in `ROUTES`) stated only by the unwrap. | S |
