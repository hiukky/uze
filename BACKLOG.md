...
| 9 | Four copies of "step an index within a list" with signed casts (`src/ui/model.rs` selection movers) | All guarded against an empty list today; one `step_within(current, delta, len)` removes the casts and the guard repetition. | S |
