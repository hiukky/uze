...
| Hooks | **SUPPORTED** — `settings.json` `hooks` entries running the generated `hooks/exec` wrapper (ADR-040); deterministic, real-binary verification pending in the Lab | **SUPPORTED** — `~/.codex/hooks.json` command form running the same wrapper (ADR-040) | **SUPPORTED** — owned regenerable `.ts` bridge + managed `plugin` entry (ADR-033); degraded for `Stop`, unsupported for `ask` | **SUPPORTED** — named entries merged into the shared `hooks.json`, running the same wrapper (ADR-040)
