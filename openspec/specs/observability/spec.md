...
Makes what UZE did legible after the fact: every action a person takes —
a CLI command, a key or a click — is one trace that reaches the end of the
work it causes, worker threads and child processes included, readable as
text on a terminal or on a local dashboard.
