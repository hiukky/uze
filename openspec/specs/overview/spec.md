...
- **WHEN** the cwd contains neither `agents.lock` nor `agents.json`
...
- **WHEN** `agents.json` parses and every declared source directory exists
...
- **WHEN** `agents.json` parses but a declared package's source directory is missing
- **THEN** Status reports the invalid count without declaring the manifest invalid

#### Scenario: Invalid manifest
- **WHEN** `agents.json` is missing, malformed, or structurally invalid
...
- **WHEN** both `agents.lock` and `agents.json` exist at the workspace root
