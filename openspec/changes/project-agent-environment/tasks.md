...
- [x] 9b.10a The Conformance Lab stages its market as a repository. `materialize_marketplace` copies the fixture, applies the run-specific substitutions, and commits — the substitutions land *in* the commit, so what a harness receives is what the repository holds. Found by the gate: all four verticals failed on the shared setup (`uze market add /work/market`) before any harness TUI launched, which is the signature of a setup death rather than a TUI bug. Matrix cells are unaffected: their overlays are applied on the host into a directory mounted read-only at the fixture path, and the copy under `/work` is what gets initialised.
