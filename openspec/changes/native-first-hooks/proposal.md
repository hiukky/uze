...
- **The wrapper is the only route.** The Rust adapters were the executable specification the wrapper templates were tested against; once the two answered every fixture identically, the answers were recorded as goldens and the runtime (`uze hook-exec`) was removed — one contract, one implementation, no `uze` on any hook's execution path. Where no template applies (Windows until a PowerShell wrapper exists), the hook is Unsupported with that reason and nothing is attached. Existing installs are re-projected on `uze update`/`uze plugin install`; nothing is migrated in place.
...
- `crates/uze-integrations`: `hooks.rs` adapters become template generators (Claude, Codex, Antigravity `sh` wrapper; OpenCode `.ts` plugin); the runtime adapters are removed once their answers are recorded as goldens; matcher translation reads the vocabulary.
- `src/main.rs`: the `hook-exec` command is removed, with the `Hooks` application service, the `HookAdapterPort` contract and the in-binary dispatcher behind it.
...
- Fixtures and docs: `conformance/_fixtures/marketplace/plugins/hook-*` handlers rewritten to the env/exit contract (the deterministic `tests/_fixtures/canonical/hook-plugin` tree went with the in-binary runtime it exercised; the wrapper's own recorded answers are what replaced it); `docs/capabilities/portable-hooks.md`; ADR-033 amended (or superseded by a new ADR); `.labs/native-hooks` retired once the generator reproduces it.
