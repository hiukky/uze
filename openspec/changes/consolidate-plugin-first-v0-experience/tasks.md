## Lifecycle and application

- [x] Add tests for add/remove/idempotency/drift and portable-only fallback.
- [x] Add managed attachment ledger and safe detach/health integration APIs.
- [x] Add package-centric application operations and CLI commands.
- [x] Add deterministic doctor and package inspection reports.
- [x] Add minimal TUI using only the application facade.
- [x] Validate Rust, OpenSpec, LikeC4, and real isolated configuration paths.
