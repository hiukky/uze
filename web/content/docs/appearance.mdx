...
| `nerd` | [Codicons](https://microsoft.github.io/vscode-codicons/dist/codicon.html), the icons VS Code draws its own chrome with, and Material Design where Codicons has nothing that fits | You installed a patched font, in any build. **Recommended.** |
...
Nerd Fonts ships each family in several builds: plain (`JetBrainsMono Nerd
Font`), **Mono** (`JetBrainsMono Nerd Font Mono`) and Propo. uze draws
correctly in all of them, and the reason is worth knowing, because it is also
why the same icon can look larger in one terminal than in another.

Every build reserves an icon **one cell**. What differs is how much of the
font's drawing fits in that cell:

- **Mono** shrinks each icon until it fits inside its one cell.
- **Plain** and **Propo** draw the icon at the font's height and let it run
  up to about 1.7 cells to the right.

uze draws every icon with a blank cell after it, so the overflow lands in
space nobody else is using and never on the text beside it. The `nerd` set is
chosen, icon by icon, so that its icons come out the same size as each other
in every build. Measured over the fonts in the catalog below, no icon is more
than 13% larger or smaller than the rest.

What the build *does* change is how large the icons are next to the text,
and it changes it the same way in every terminal measured: Mono icons come
out about a quarter smaller than plain ones (the sizes are in the table
below). If you want larger icons, use the plain build. If you want them no
wider than a letter, use Mono. Do not add a `width` override for either build: uze already reserves
the room, and an override only adds a second blank cell.

### Terminals

The table says which terminals and builds draw every icon of the `nerd` set
correctly:

- contained in the room uze reserves for it;
- not clipped;
- the same size as the others;
- with the text after it where it belongs.

Each cell counts the fonts of that build that pass, followed by the size
icons come out at, as a fraction of the line's height. It is generated from
the
[Rendering Lab](https://github.com/hiukky/uze/tree/main/conformance/rendering)'s
evidence. The Lab runs each terminal in a disposable environment, draws `uze
theme specimen nerd` in every font of its catalog, and measures the pixels.

{/* rendering-catalog:begin — generated by conformance/rendering/catalog.py */}

| Terminal | Version | Plain | Mono | Propo | Evidence |
| --- | --- | --- | --- | --- | --- |
| alacritty | 0.15.1 | ✓ 7/7 · 0.66 | ✓ 7/7 · 0.48 | ✓ 6/6 · 0.66 | Rendering Lab |
| foot | 1.21.0 | ✓ 7/7 · 0.66 | ✓ 7/7 · 0.48 | ✓ 6/6 · 0.66 | Rendering Lab |
| ghostty | 1.3.1-dev | ✓ 7/7 · 0.67 | ✓ 7/7 · 0.50 | ✓ 6/6 · 0.67 | Rendering Lab |
| kitty | 0.41.1 | ✓ 7/7 · 0.67 | ✓ 7/7 · 0.48 | ✓ 6/6 · 0.69 | Rendering Lab |
| konsole | 25.04.2 | ✓ 7/7 · 0.68 | ✓ 7/7 · 0.52 | ✓ 6/6 · 0.68 | Rendering Lab |
| vte (xfce4-terminal) | 1.1.4 | ✓ 7/7 · 0.66 | ✓ 7/7 · 0.48 | ✓ 6/6 · 0.66 | Rendering Lab |
| wezterm | 20240203 | ✓ 7/7 · 0.67 | ✓ 7/7 · 0.50 | ✓ 6/6 · 0.67 | Rendering Lab |
| xterm | 398 | ✓ 7/7 · 0.65 | ✓ 7/7 · 0.48 | ✓ 6/6 · 0.66 | Rendering Lab |

{/* rendering-catalog:end */}

A terminal the Lab cannot run can still be measured. Run `uze theme specimen
nerd`, take a screenshot, and pass it to the analyzer:
