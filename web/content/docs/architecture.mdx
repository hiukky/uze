...
graph TD
    ...
    THEME["uze-theme<br/>the design vocabulary: tokens, symbols, resolver"]
    KEYS["uze-keys<br/>actions · scopes · chords"]
    DOC["uze-document<br/>the shape a record declares, and its ladder"]
...
- **`uze-extensions`** — built-in TUI extensions. An extension answers with a
  ...
  from plugin bytes a harness reads (ADR-041). Two ship today:
  [Code and Architect](/docs/workspace/extensions).
...
- **`uze-keys`** — actions, scopes and chords: the vocabulary
  [Shortcuts](/docs/configuration/keys) is written in, and the resolver that answers which
