...
<Files>
  <Folder name="crates" defaultOpen>
    ...
    <Folder name="uze-application — product facade, lifecycle orchestration" />
    <Folder name="uze-integrations — one module per harness + shared helpers" />
  </Folder>
  <Folder name="src — CLI parsing, TUI, runtime PATH shim, performance taxonomy" />
  <Folder name="tests — domain-organized integration suites (L0–L4)" />
  <Folder name="conformance — Harness Conformance Lab (Python, disposable Docker)" />
  <Folder name="docs" defaultOpen>
    <Folder name="adr — architecture decision records (numbered)" />
    <Folder name="architecture — invariants the architecture holds today" />
  </Folder>
  <Folder name="openspec — spec-driven change proposals" />
  <Folder name="plugins/uze — the official /uze Skill (this repo is uze's marketplace)" />
</Files>
