...
checkout — see [Running agents](/docs/workspace/agents). The layout of those
checkouts (`.worktrees/<id>`, branch `agent/<id>`) is fixed infrastructure and
is deliberately *not* declared here.

| Key | Default | Meaning |
| --- | --- | --- |
| `default` | `in-place` | Where an agent launched here starts: `in-place` in the space's own root, or `isolated` in a checkout of its own |
