...
uze market add <source>            # register a marketplace (a Git URL, or a local clone)
...
marketplace's mirror — a bare `--filter=blob:none` clone uze keeps per
...
uze theme specimen [set] # every symbol of a set in the slot uze draws it in, to screenshot
```

The palette and the glyph set are separate choices, and neither changes the
other. Machine-level, and `--format json` on `list`, `show`, `glyphs` and
`specimen`, whose JSON is the layout the Rendering Lab's analyzer measures a
screenshot of it against. See [Appearance](/docs/appearance).
