...
uze install [path]           # resolve agents.yaml into agents.lock, then install it
...
uze theme glyphs <set>  # draw every mark from that set, whatever theme is on
```
...
<Callout type="info">
  `terminal` and `theme` are not listed in `uze --help` yet — the terminal
  runtime is experimental, and a theme is normally chosen from the TUI.
</Callout>
