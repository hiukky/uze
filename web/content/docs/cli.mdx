...
Two scopes, and they never touch each other's state. Project commands
...
client. These manage that session directly — running `uze` with no
...
served from the harness-detection cache) or `JustifiedSlow` with a
