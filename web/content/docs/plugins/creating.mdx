...
uze is **Plugin First, Capability Aware**: the package you author is the distribution
...
see [Capabilities](/docs/concepts/delivery#capabilities) for the per-harness route.
...
Only `name`, `description`, and the body are claimed as the portable subset. Claude
...
- **Effect**: `observe` (default), `allow`, `ask`, `deny`, or `transform` (`PreToolUse`
  ...
  [what each harness can actually carry](/docs/concepts/delivery#capabilities#what-each-harness-can-actually-carry).
...
`marketplace.json` was briefly named `agents.json` and reverted — the same manifest
